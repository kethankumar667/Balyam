...
import type {
  ...
  SamethaluOptions,
  SamethaluPhase,
  SamethaluPlayerPublic,
  SamethaluPlayerState,
  SamethaluPublicState,
  SamethaluQuestion,
  SamethaluStanding,
} from "@shared/types.js";
import { DEFAULT_SAMETHALU_OPTIONS } from "@shared/types.js";
import { SAMETHALU_QUESTIONS } from "./questions.js";
...
export class SamethaluEngine implements GameEngine {
  ...
  private phase: SamethaluPhase = "playing";
  private round = 1;

  private questionPool: SamethaluQuestion[] = [];
  private currentQuestion: SamethaluQuestion | null = null;
  ...
  private standings: SamethaluStanding[] | null = null;
  ...
  setOptions(opts: Partial<SamethaluOptions>): void {
    this.pendingOptions = { ...DEFAULT_SAMETHALU_OPTIONS, ...opts };
  }

  init(players: Player[]): void {
    if (players.length < this.minPlayers || players.length > this.maxPlayers) {
      throw new Error(`Samethalu Quiz requires ${this.minPlayers}-${this.maxPlayers} players`);
    }
    this.opts = this.pendingOptions ?? { ...DEFAULT_SAMETHALU_OPTIONS };
  ...
  private computeStandings(): SamethaluStanding[] {
  ...
  private publicPlayers(): SamethaluPlayerPublic[] {
  ...
  getPublicState(): SamethaluPublicState {
    ...
    const questionPub = this.currentQuestion
      ? {
          ...
          proverb: this.currentQuestion.proverb,
  ...
  getStateFor(playerId: string): SamethaluPlayerState {
  ...
  applyAutoMove(playerId: string): MoveResult {
    if (this.phase === "playing" && this.currentQuestion) {
      // Bots pick correct option 80% of the time, else random
      const isCorrect = this.rng() < 0.8;
