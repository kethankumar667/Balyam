...
- **R2.3** Sockets: the main socket layer (`server/src/sockets/index.ts`) has **no verified identity today**. Only Mandali verifies, through `mandali:authenticate` + `resolvePlayerIdentity`. WP3.0 below builds a general version. **WP3.0 is done:** `session:authenticate` binds a verified identity to the socket (`server/src/sockets/socketSession.ts`). A social socket event takes its actor from `requireSocketIdentity(socket)` / `requireSocketIdentityOrAck(socket, ack)` and from nothing in its payload; a connection with no live session is refused with `UNAUTHENTICATED`.
