...
/** 7-player heptagon Ludo board — geometry only (drawn by BoardView). */
...
export function buildBoard7(): LudoBoard {
  ...
  colors.forEach((color, i) => {
    ...
    wedges.push({
      ...
      podCenter,
      nameAnchor: P(R_POD - POD_R + 1.6, a),
  ...
  return {
    ...
    colorStarts,
    safeSquares,
    framePoly: outline.map((p) => scale(p, 1.065)).map(fmt).join(" "),
    boardPoly: outline.map(fmt).join(" "),
    wedges,
    centerBadge: colors.map((_, i) => P(5.0, i * W)).map(fmt).join(" "),
  };
}
