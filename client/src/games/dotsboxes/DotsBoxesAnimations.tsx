...
/**
 * Box Captured — "MINE!" Animation.
 */
export function DotsBoxesMineBurst({
  playerName,
  penColor,
  onComplete,
}: {
  ...
  useEffect(() => {
    fireStarSparkleBurst({ left: "50%", top: "40%" }, { intensity: 0.8 });
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-40 flex items-center justify-center">
      ...
      >
        ...
        <ComicBurstText text="MINE!" accent={penColor ?? "#1E293B"} fill="#FDE047" seed={9} />
        <div
          className="px-3.5 py-1 rounded-full text-xs font-black text-white shadow-xl tracking-wider uppercase"
          style={{ background: penColor ?? "#2563eb" }}
        >
          {playerName} captured a box!
        </div>
      </motion.div>
    </div>
  );
...
/**
 * Dots & Boxes Winner Celebration — L4 Box Master Victory Ceremony.
 */
export function DotsBoxesWinnerCelebration({
  winnerName,
}: {
  winnerName: string;
}) {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        <ComicBurstText text="BOX MASTER!" accent="#1E293B" fill="#FDE047" seed={25} />
