  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="flex-1 min-h-0 overflow-y-auto overscroll-contain p-2.5 sm:p-3 space-y-2 custom-scrollbar focus:outline-none focus-visible:ring-2 focus-visible:ring-[#EA5A1F]/30"
        tabIndex={0}
      ...
      >
        {messages.length === 0 && (
          <div className="text-[#6E5336] dark:text-slate-400 text-xs py-6 text-center font-medium">
        ...
        {messages.map((m) => {
          ...
          return (
            <div
              key={m.id}
              className={`text-xs p-2.5 sm:p-3 rounded-2xl max-w-[88%] break-words shadow-2xs transition-all ${
                isSelf
                  ? "ml-auto bg-gradient-to-r from-emerald-600 to-emerald-700 text-white rounded-br-xs"
                  : "mr-auto bg-white dark:bg-[#182234] text-[#2B3550] dark:text-slate-100 border border-[#EEDBCA]/80 dark:border-slate-700/80 rounded-bl-xs"
              }`}
            >
              <div
                className={`text-[10px] font-black mb-0.5 ${
                  isSelf ? "text-emerald-100" : "text-[#5C4328] dark:text-slate-300"
                }`}
              >
                {isSelf ? "You" : m.playerName}
              </div>
              <div className="leading-relaxed whitespace-pre-wrap select-text font-medium">
                {m.text}
              </div>
            </div>
          );
        ...
        <div className="flex flex-wrap gap-1.5 max-h-[64px] sm:max-h-[72px] overflow-y-auto custom-scrollbar py-0.5">
          {QUICK_PHRASES.map((p) => (
            <button
              ...
              type="button"
              onClick={() => sendText(p)}
              className="min-h-[38px] sm:min-h-[34px] px-3 py-1.5 rounded-full bg-white dark:bg-[#1F2B3E] hover:bg-[#FFF4E0] dark:hover:bg-[#283850] active:scale-95 text-[#5C4328] dark:text-slate-200 border border-[#EEDBCA] dark:border-slate-700 text-[11px] font-bold transition cursor-pointer shrink-0 shadow-2xs focus:outline-none focus-visible:ring-2 focus-visible:ring-[#EA5A1F]"
            >
              {p}
            </button>
          ))}
      ...
      >
        <div className="flex items-center gap-1.5 sm:gap-2 max-w-full relative">
          ...
          {text.length >= 400 && (
            <span
              className={`absolute right-14 sm:right-20 top-1/2 -translate-y-1/2 text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${
                text.length >= 480
