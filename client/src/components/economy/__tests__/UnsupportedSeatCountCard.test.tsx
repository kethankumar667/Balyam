...
describe("UnsupportedSeatCountCard Component", () => {
  it("renders corrective message and seat count for 13-seat table as host", () => {
    render(<UnsupportedSeatCountCard seatCount={13} isHost={true} />);

    expect(screen.getByRole("alert")).toBeDefined();
    expect(screen.getByText(/Unsupported Table Size/i)).toBeDefined();
    expect(screen.getByText("(13 seats)")).toBeDefined();
    expect(screen.getByText(/Max 12 Seats Supported/i)).toBeDefined();
    expect(
      screen.getByText(/game economy currently supports tables of 1 to 12 players/i)
    ).toBeDefined();
  ...
  it("renders correct pluralization when multiple excess players need to be removed", () => {
    render(<UnsupportedSeatCountCard seatCount={15} isHost={true} />);

    expect(screen.getByText("(15 seats)")).toBeDefined();
  ...
  it("renders respectful waiting message for non-host participants", () => {
    render(<UnsupportedSeatCountCard seatCount={14} isHost={false} />);

    expect(
      screen.getByText(/Waiting for host to adjust table capacity to 12 or fewer players/i)
    ).toBeDefined();
