...
export function CosmeticsItemCard({
  ...
  const cardSurface = isSelected
    ? `${rarity.borderSelected} ring-1 ring-amber-400/80 bg-gradient-to-br from-amber-50 via-white to-stone-50 dark:from-[#201805] dark:via-[#121724] dark:to-[#080b12] shadow-[0_24px_54px_-30px_rgba(245,158,11,0.8)] ${rarity.surfaceSelected}`
    : `${rarity.border} ${rarity.borderHover} bg-gradient-to-br from-white via-stone-50 to-amber-50/[0.35] dark:from-[#111827] dark:via-[#0b1120] dark:to-[#05070d] shadow-[0_18px_38px_-32px_rgba(15,23,42,0.8)]`;
  ...
  return (
    <div
      ...
      className={`group relative p-4 rounded-[22px] border flex flex-col justify-between gap-3.5
                  ...
                  ${cardSurface}`}
    >
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(251,191,36,0.16),transparent_34%),radial-gradient(circle_at_88%_18%,rgba(125,211,252,0.10),transparent_30%)] opacity-80" />
      <div aria-hidden="true" className="pointer-events-none absolute inset-x-3 top-2 h-px bg-gradient-to-r from-transparent via-white/50 to-transparent dark:via-white/[0.18]" />
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 opacity-[0.05] bg-[linear-gradient(135deg,rgba(255,255,255,0.9)_1px,transparent_1px)] [background-size:12px_12px]" />
      ...
      <div className="flex items-center justify-between gap-2 relative z-10">
        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${rarity.badge}`}>
          {item.rarity}
        </span>

        {/* Presentation State Badge */}
        {pres.state === "EQUIPPED" ? (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-black bg-emerald-100 text-emerald-700 border-emerald-300 dark:bg-emerald-500/20 dark:text-emerald-400 dark:border-emerald-500/40 border flex items-center gap-1 shrink-0">
            <Check className="w-3 h-3 stroke-[3]" /> EQUIPPED
          </span>
        ) : pres.state === "EQUIPPING" || pres.state === "PURCHASING" ? (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-black bg-amber-100 text-amber-800 border-amber-300 dark:bg-amber-500/20 dark:text-amber-300 dark:border-amber-500/40 border motion-safe:animate-pulse flex items-center gap-1 shrink-0">
            <RefreshCw className="w-3 h-3 motion-safe:animate-spin" /> {pres.badgeLabel}
          </span>
        ) : isAdminUser ? (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-black bg-amber-100 text-amber-800 border-amber-300 dark:bg-amber-500/20 dark:text-amber-400 dark:border-amber-500/40 border flex items-center gap-1 shrink-0">
            <Crown className="w-3 h-3" /> FREE
          </span>
        ) : pres.state === "OWNED" ? (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-sky-100 text-sky-700 border-sky-300 dark:bg-sky-500/20 dark:text-sky-400 dark:border-sky-500/40 border shrink-0">
            OWNED
          </span>
        ) : pres.state === "DEFAULT" ? (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-stone-200 text-stone-600 border-stone-300 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700 border shrink-0">
            DEFAULT
          </span>
        ) : pres.state === "LOCKED" ? (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border-amber-300 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/30 border flex items-center gap-1 shrink-0">
            <Lock className="w-3 h-3" /> Day 7
          </span>
        ) : pres.state === "AVAILABLE" ? (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-bold bg-amber-50 text-amber-800 border-amber-300 dark:bg-amber-500/15 dark:text-amber-300 dark:border-amber-500/30 border flex items-center gap-1 shrink-0">
            <Coins className="w-3 h-3 text-amber-600 dark:text-amber-400" />
            {item.priceCoins.toLocaleString()}
          </span>
        ) : (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-bold bg-rose-50 text-rose-700 border-rose-300 dark:bg-rose-950/60 dark:text-rose-400 dark:border-rose-800/40 border shrink-0">
      ...
      <div className="flex items-center gap-3 relative z-10">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-stone-600 dark:text-zinc-400 line-clamp-1 mt-1">
            {item.description}
          </p>
        </div>
      </div>
      ...
      <div className="pt-3 border-t border-stone-200/80 dark:border-white/10 flex items-center justify-between gap-2 text-xs relative z-10">
        {pres.state === "INSUFFICIENT_BALANCE" ? (
          <div className="flex-1 min-w-0">
            ...
            <div className="w-full h-1.5 rounded-full bg-stone-200 dark:bg-black/40 overflow-hidden border border-white/10">
              <div
