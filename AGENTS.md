...
| Auth | **Seats are authenticated; accounts are asserted, not verified.** A seat is identified by a public `playerId` (broadcast in `room:state`) and *owned* via a server-signed `seatToken` (`server/src/lib/seatToken.ts`), issued only in the `room:create` / `room:join` ack and stored per room code under `mpg.seats` — that part is real cryptography. On top of it sits a guest/member distinction (`client/src/store/authStore.ts`, key `bhalyam.account`) which is **device-local and unverified**: signing in checks no password because there is no account store to check one against. It exists so the guest permission model below is wired end to end; it protects nothing. Google/Apple buttons are honestly inert pending OAuth credentials. |
...
- **`hostKind` / `accountKind` default OPEN, not closed.** Only an explicit `"guest"` seals a room or marks a seat. This is the opposite of the usual fail-closed instinct and is justified in `createRoom`'s comment: the flag is not a security boundary, so the failure that matters is a caller which has not been taught the field silently losing the ability to play with anyone.
