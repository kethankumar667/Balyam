# AGENTS.md — BHALYAM AI Engineering Operating System & Governance

> **MANDATORY FOR ALL AI AGENTS & CODING ASSISTANTS:**  
> This repository operates under strict engineering governance. All AI agents, contributors, and assistants **MUST** read this file and follow the required engineering standards before proposing requirements, writing code, executing refactors, or generating reviews.

---

## 0. Mandatory Document Loading Order

Before executing any engineering task in BHALYAM, AI agents must read and internalize the repository intelligence documents in this precise sequence:

### Primary Tier: Product & Architectural Intelligence
1. [`docs/ai/bhalyam-domain-knowledge.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/bhalyam-domain-knowledge.md) — What BHALYAM is, target audience, game catalog, and core value propositions.
2. [`docs/ai/bhalyam-product-vision.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/bhalyam-product-vision.md) — 5 Product Tenets, retention loops, anti-feature-creep philosophy.
3. [`docs/ai/bhalyam-platform-rules.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/bhalyam-platform-rules.md) — The 10 Inviolable Platform Laws (zero `any`, server authority, dual layouts).
4. [`docs/ai/bhalyam-design-system.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/bhalyam-design-system.md) — DLS tokens, Standard Buttons, Cards, Modals, Empty/Loading/Error states.
5. [`docs/ai/bhalyam-game-framework.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/bhalyam-game-framework.md) — 12 mandatory game capabilities, state shapes, bot automation.
6. [`docs/ai/bhalyam-decision-log.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/bhalyam-decision-log.md) — Architectural Decision Records (ADRs) explaining *why* decisions were made.

### Secondary Tier: Engineering Standards & Quality Protocols
7. [`docs/ai/antigravity.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/antigravity.md) — 11-phase engineering thinking framework, multi-persona cognitive review.
8. [`docs/ai/frontend-standards.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/frontend-standards.md) — React 18, strict TypeScript (0 `any`), Zustand stores, dual layouts.
9. [`docs/ai/architecture-principles.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/architecture-principles.md) — In-memory `RoomManager`, modular `GameEngine`, 10-year maintainability.
10. [`docs/ai/ui-ux-standards.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/ui-ux-standards.md) — DLS tokens, gaming aesthetics, 44x44px touch targets, SkeletonLoader, EmptyState.
11. [`docs/ai/accessibility-standards.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/accessibility-standards.md) — WCAG 2.1 AA, `:focus-visible` golden rings, screen reader announcements.
12. [`docs/ai/performance-standards.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/performance-standards.md) — < 220kB initial bundle, 60fps GPU transforms, lazy-loaded game boards.
13. [`docs/ai/security-standards.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/security-standards.md) — Closed-set payload sanitization, cryptographic HMAC `seatToken`, zero raw HTML.
14. [`docs/ai/testing-standards.md`](file:///c:/Users/GontlaKethanKumar/Desktop/copilot_workshop/copilot_training/MultiplayerGames/docs/ai/testing-standards.md) — Vitest coverage, zero-skip policy, deterministic engine tests, mobile matrix.
...
**BHALYAM** is a web-based multiplayer game lounge. Players land on the home page, pick a tile (Hand Cricket, Ludo, Snakes & Ladders, Rummy, RPS, UNO, Word Building, Dots & Boxes, Star Game, Bingo), and either create a 6-character room code or join an existing one. Once in a room they can chat, voice-chat (mesh WebRTC), invite bots, run "pass & play" local seats, and play a server-authoritative round of the chosen game with a rematch loop.
...
| Accounts SDK | `@supabase/supabase-js` 2 (client only; the server verifies tokens with `node:crypto` and `fetch`, and takes no Supabase dependency) |
...
| Database | **Supabase (Postgres), for accounts only — and optional.** One table, `public.profiles` (display name + avatar), created by `supabase/migrations/0001_accounts.sql` and reached from the CLIENT via `@supabase/supabase-js`. Room and game state remain entirely in-memory in `RoomManager`; the server holds no DB connection and needs no `DATABASE_URL`. With `VITE_SUPABASE_*` unset the whole layer no-ops and the app behaves exactly as it did before accounts existed. |
| Auth | **Seats are authenticated. Accounts are verified when Supabase is configured, asserted when it is not.** A seat is identified by a public `playerId` (broadcast in `room:state`) and *owned* via a server-signed `seatToken` (`server/src/lib/seatToken.ts`), issued only in the `room:create` / `room:join` ack and stored per room code under `mpg.seats` — real cryptography, independent of everything below. On top of it sits the guest/member distinction (`client/src/store/authStore.ts`): with Supabase keys set it comes from a real session (email+password or Google, session under `bhalyam.session`) and the client sends `accessToken` on create/join, which `server/src/lib/supabaseAuth.ts` verifies before believing `hostKind`; with no keys it falls back to the old device-local `bhalyam.account` flag, which checks nothing and says so on every screen. `GET /health` reports which mode the server is in under `auth`. See `docs/runbooks/supabase.md`. |
| Permissions | **`shared/permissions.ts` is the single source of truth for who may do what.** `capabilitiesFor(kind)` returns the capability set; every gate in the app reads it rather than testing `isGuest` inline. The product rule is *a guest can play, a guest cannot gather*: solo-vs-bots, Pass & Play, profile editing, voice and accepting an invite are open to everyone; hosting a shareable room, typing a room code, and Party Mode (`/tv/:code`) need an account. Enforced server-side by `Room.sealed` — see §18. |
| Player-supplied values | **Anything a client sends that the server re-broadcasts is an injection point into every other player's page, and must be validated against a closed set on the way in.** Two live examples: reactions (`shared/reactions.ts`, `ALLOWED_REACTIONS`) and avatars (`shared/avatars.ts`, `sanitizeAvatar`) — the latter because `Player.avatar` becomes an `<img src="/Avatars/…">` on everyone's device, so an unchecked string could climb out of the folder or resolve off-site and leak every player's IP. Validate with set membership, never a permissive pattern, and drop silently rather than erroring. |
...
│           ├── stargame/StarGameEngine.ts
│           └── bingo/BingoEngine.ts
└── shared/
...
- **Turn timers** are managed in `RoomManager` (one `turnTimer` per room). Bingo reuses it as the number-caller interval rather than a per-player deadline, which is why it is excluded from the idle-takeover strike count.
...
- **Pass & Play allow-list** (in `RoomManager.ts`): only `ludo`, `snl`, `wordbuilding`, `dotsboxes`. Adding a game to this list requires confirming it does **not** show hidden information that would leak across local seats on a shared device.
...
- **Sealed rooms** (`Room.sealed`, mirrored onto `RoomPublicState.sealed`). A room opened by a guest takes no new arrivals — `joinRoom` and `spectateRoom` both refuse with `SEALED_ROOM_ERROR`. Three things are deliberate and easy to break:
  1. The seal check sits **after** the seat-reclaim and idempotency branches, so a sealed room still lets its own host back in on refresh and still honours a seat token. Moving it earlier locks players out of their own game.
  2. Sealing is **one-way** (`sealRoom`, no `unsealRoom`), so a code that was ever refused stays refused.
  3. Host migration prefers a member, then any human, then whatever is left; if a guest inherits the room it is sealed on the way. See `guestSealedRoom.test.ts`, which pins all of this.
- **`hostKind` / `accountKind` default OPEN, not closed.** Only an explicit `"guest"` seals a room or marks a seat. This is the opposite of the usual fail-closed instinct and is justified in `createRoom`'s comment: the failure that matters is a caller which has not been taught the field silently losing the ability to play with anyone. **This applies only in `auth: off` mode.** Once `server/src/lib/supabaseAuth.ts` is configured, `resolveAccountKind` runs first and inverts the default: an unverifiable claim of membership becomes `"guest"`. Both handlers in `sockets/index.ts` are `async` for exactly this reason, and `RoomManager` still receives a settled `AccountKind`, which is why its tests need no tokens.
- **Accounts are client-side; the server only checks tokens.** `client/src/lib/supabase/` holds the whole account layer. The server never opens a DB connection, never sees a password, and takes no Supabase dependency — `supabaseAuth.ts` verifies an HS256 JWT with `node:crypto`, or asks `/auth/v1/user` and caches the answer. Three modes (`jwt-secret`, `auth-api`, `off`) reported by `/health`. Do not add `@supabase/supabase-js` to `server/`.
- **The Supabase session key is pinned to `bhalyam.session`.** supabase-js would default to `sb-<projectRef>-auth-token`, whose name changes with the project — and the DPDP inventory (`dataInventory.ts`) has to declare every key by name. If you change `storageKey`, change the inventory in the same commit, and bump `NOTICE_VERSION`.
- **`createRoom`'s parameter list is positional and 24 long.** `avatar` is no longer last — `hostKind` follows it. `avatarSharing.test.ts` derives its padding from `createRoom.length` and now subtracts `PARAMS_AFTER_AVATAR`; bump that constant if anything else is appended, or the test will silently pass a filename into the wrong slot.
...
- **Memory Match no longer exists.** It was once a server-only engine with no client board. Engine, types, registry entry and RoomManager wiring have all been removed — `GameKind` has ten members and none of them is `memorymatch`. Older docs in this repo still mention it; they are stale. Do not "finish" it on the strength of a stale reference.
- **The single-player Cricket app has been removed.** `client/src/games/cricket/` (66 files) and its `/cricket/*` route are gone. It was a local-only interface with no Socket.IO involvement, reachable only by typing the URL — no lobby tile ever linked to it. Hand Cricket (`handcricket`) is a different, fully multiplayer game and is unaffected.
...
- **Production environment variables** beyond those in `server/.env.example` (`PORT`, `CLIENT_ORIGIN`, `SESSION_SECRET`, `SUPABASE_JWT_SECRET` / `SUPABASE_URL` + `SUPABASE_ANON_KEY`, `TURN_*`, `KEEPALIVE_URL`) and `client/.env.example` (`VITE_SERVER_URL`, `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`). If others exist on Render/Vercel, they are not declared here.
...
- **Match history / durable game results** — not built. `supabase/migrations/` has accounts only; the reasons that is where it stops are in `docs/runbooks/supabase.md`.
